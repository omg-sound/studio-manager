...
- 목록=한 줄 요약 카드 + 검색(`?q=`). **진행 중/완료 2단 분리**(2026-07-02): 목록을 **진행 중(상단)**·**완료(하단 접기 `<details>`)**로 나눈다. 완료 섹션 summary는 **명확한 클릭 바**(테두리+`bg-surface`+호버 `bg-elevated`/`border-primary` + 배지 개수 + **'펼치기/접기' 라벨**+chevron `group-open` 회전) — 그냥 제목처럼 보여 안 눌리는 문제 개선(사용자 피드백). **완료 판정**=`listProjects`가 파생하는 `is_completed` = **다가오는 세션 없음**(오늘 이후·취소 제외 `upcoming_cnt=0`) **AND 미완료 작업 없음**(`status<>'Completed'` = 대기·진행중 `open_tasks=0`) **AND 실제 활동 있었음**(세션 또는 작업 존재 `content_cnt>0` — 빈 새 프로젝트는 진행 중). 즉 믹싱 전용도 전 곡 작업 완료면 완료, 믹싱 대기면 진행 중, 녹음 세션 다 지났고 후반작업 없으면 완료. 검색 중이거나 진행 중이 0건이면 완료 섹션 자동 펼침(`projects.routes` GET `/`, 사용자 요청). **결제 예정 금액 = 작업액 + 세션액**(작업액=Σ`COALESCE(NULLIF(total_price,0), task_types.unit_price)` — **확정 금액 없으면 종류 기본단가로 폴백**, 청구 폼 기본값과 동일; 세션액=`sessionAmountsByProject`). 2026-07-01 수정: 단가표 가격이 0일 때 만든 작업은 `total_price=0`이라 목록에서 빠지던 것(녹음 세션은 들어오는데 믹스 등 작업이 누락) → 종류 기본단가 폴백으로 청구 폼과 일치.
