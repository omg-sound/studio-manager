...
test("드롭다운·FOUC 스크립트 배선(views.js)", () => {
  assert.match(VIEWS, /data-theme-select/, "테마 선택 드롭다운 마커");
  for (const opt of ["original", "apple", "material", "linear"]) {
    assert.match(VIEWS, new RegExp(`value="${opt}"`), `${opt} 옵션`);
  }
...
test("app.js: 팔레트 드롭다운 처리 + localStorage + Linear 다크 기본", () => {
  assert.match(APP, /data-theme-select/, "app.js가 드롭다운 처리");
  assert.match(APP, /localStorage\.setItem\("palette"/, "palette 저장");
  assert.match(APP, /"linear".*setTheme\("dark"\)|linear.*dark/s, "Linear 선택 시 다크 기본");
});
