...
- **이번 세션 — 입력 콤보 전면 통일 + Drive 스튜디오 계정 고정 + 첨부 저장 안전화 + 목록 링크 통일**: **프로젝트 폼 4개 입력(아티스트·소속사/레이블·제작사/운영사·고객측 담당자)을 전부 같은 흐름의 검색 콤보 + 간이 등록 모달로 통일** — `companyCombo`(app.js `[data-company-combo]`, 업체명 TEXT·역할 겸업·사업자·대표·이메일·전화, `ensureCompanyParty` 이름 dedup)·`personCombo`(`[data-person-combo]`, hidden contact_id·이름/전화/이메일/회사/직책·소속 반영·선택 정보표시), 각각 fetch 생성(POST /clients·/contacts가 `X-Requested-With:fetch`면 JSON). 네이티브 datalist 기반 `contactCombo` 폐기(죽은코드). 아티스트 콤보: **본명=입력칸 안 오버레이 표시(입력 아님, `data-artist-realname` absolute·pointer-events-none)**·**본명으로도 검색/정확일치 링크**(옵션 realName)·**그룹 체크박스 제거**(모달/연락처에서 생성)·**선택 시 드롭다운 닫힘**(pick이 hide→fireInput 순서라 재렌더로 다시 열리던 버그: fireInput 후 hide로 수정, 3콤보 공통). '🔍 이름을 입력해 검색' 안내줄 폐기. **Drive 자료 저장을 studio@omgworks.kr로 영구 고정**: `config.studioDriveEmail`(env `STUDIO_DRIVE_EMAIL` override) — **그 이메일로 로그인할 때만** refresh token 저장(치프 역할 무관·치프 바뀌어도 고정), 누가 로그인하든 업로드는 단일 저장 토큰(`driveClient`) 사용이라 자료는 항상 이 계정 Drive 한 곳. `/auth/google?drive=1`(설정 연결 버튼)이 `prompt=select_account+consent`·`login_hint`로 계정 선택기 강제(활성 세션이 다른 계정이어도 스튜디오 계정 선택). 설정에 연결 계정 표시·불일치 시 경고. (근본 버그: OAuth 콜백이 역할 조건 없이 모든 로그인에서 토큰 덮어써 마지막 로그인한 사람 개인 Drive로 바뀌던 것 — 폴더 중복·'파일 없음'·로컬 폴백 유발. 중간에 '치프만 저장'으로 완화 후 스튜디오 계정 고정으로 확정.) **첨부 저장 안전화**: `storage.put`이 Drive 연결 상태에서 업로드 실패 시 조용히 로컬 폴백하던 것 → 예외(`DRIVE_UPLOAD_FAILED`)로 알림(미연동만 로컬), 첨부 행에 **저장 위치 배지(Drive/로컬 저장)**, '등록증 없음' 배지 앰버(`badge-warning`). **연락처 목록을 클라이언트 목록과 동일 흐름으로**: 이름(→/contacts/:id)·소속 회사(→/clients/:client_id) 각각 링크·직함 텍스트·전화/이메일 우측 비링크(복사 가능)·배지 아래 줄(`listRowLinked` 단일링크 → 수동 다중링크). **입력 콤보 전면 통일(4단계)**: `personCombo`를 **views.js 공용**으로 추출·파라미터화(idField·nameField·selectedId·options·compact) + app.js를 `initOne`+**`window.__initPersonCombos(container)`**(동적 행 초기화·`root.__pcInit` 멱등)로 리팩터 → **프로젝트 고객측 담당자·클라이언트 상세 담당자·세션 담당 디렉터(동적 다중 행, '+추가' clone도 초기화)**가 전부 같은 검색+모달 콤보. **청구처는 `payerCombo`(views.js·`[data-picker-combo]`)** — 클라이언트+담당자 이중 hidden id(client_id/payer_contact_id), 커스텀 팝업·선택 시 닫힘, 생성 모달 없음(기존에서 고름), from-tasks·수동 청구 폼 공용. 옛 datalist 콤보(`[data-client-combo]`/`[data-contact-combo]`)와 app.js `syncCombo` 위임 핸들러·`clientCombo`/`clientComboLabel` **전부 제거**(죽은 코드). 필드명 불변이라 서버 로직 무변경. 종단 스모크: payerCombo→invoice.payer_id 정확, 디렉터 personCombo→기존(director_contact_id)+새 이름(director_name) 생성·연결. 각 응집 커밋(main 자동배포)·53 테스트.
- **직전 세션 — 당사자 모델 정합 마무리 + Drive 중복폴더 수정 + 목록 UI 정리 + 그룹 개념/멤버·유형 흐름 재편**: (당사자 Party 모델 P1~P4b 컷오버·레거시 드롭·compat 제거는 직전 컨텍스트에서 완료·배포). **Drive 루트 폴더 중복 생성 방지**: `ensureFolder`가 캐시 미스 시 이름으로 기존 폴더 검색 후 재사용(가장 오래된 원본), `reconcileRootFolder`+`/drive-check`가 중복 감지·원본 캐시 재지정 — `drive.file` 토큰 변경으로 원본 폴더 미검색·빈 중복 생기던 것 해결(사용자 Drive에서 실증). **클라이언트·연락처 목록 행 정보 재배치**: 회사/대표 `text-xs`, 우측 전화 위·이메일 아래 세로 분리(업체는 사업자→전화→이메일), **이름만 링크**(`listRowLinked` — 우측 이메일·전화·사업자 드래그·복사 시 상세 미이동). **클라이언트 '그룹' 분류 표면화**(kind='group' 탭·배지) → **그룹↔소속 멤버 연결**(`parties.group_id`·그룹 상세 멤버 관리·아티스트/연락처 상세 소속 그룹 select) + **새 클라이언트 유형 3택 흐름**(업체/아티스트/그룹, '기타' 폐기, '제작사'→'제작사/운영사' 표기) — 위 클라이언트 섹션 참조. **이어서**: 소속사 select(아티스트·그룹, affiliations 재사용)·**그룹↔멤버 소속사 연동**(상속·전파·오버라이드, party.test +1) / **관계자 탭** 신설(대표·A&R·디렉터 등 사람·비아티스트, `listAssociates`)·**새 클라이언트 드롭다운**(CSP-safe `<details>`) / 상세 편집 자기-링크 '돌아가기' 제거·**백링크 확대**(text-base·관계자→클라이언트 복귀 `?from=`) / **Drive 하위 폴더 중복 방지**(`ensureSubfolder` 이름 검색·reconcile 하위 통합) / **첨부 깨진 링크 감지**(`storage.exists`) / **flash 토스트화**(fixed·레이아웃 밀림 제거) / **캘린더 이벤트에 디렉터 추가** + **동기화 진단**(`calendar.syncStatus`·저장 시 연동 실패 안내·`syncSessionEvent` try/catch fail-safe). **유지보수 점검**(전 모듈 로드·53 테스트·빌드·전 페이지/생성/상세/404/삭제 E2E 스모크 무에러 + 죽은 import 7개 정리). 각 응집 커밋(main 자동배포).
- **직전 세션 — data.js 도메인 모듈 분리 완료 + 청구·세션 실앱 E2E 검증**: `data.js` **2049→58줄** 함수 본문 0의 **순수 재export 허브** + **14개 도메인 모듈**(`src/data/*.js`: studio·client-files·revenue·deliverables·rooms·rate-items·task-types·contacts·clients·projects·tracks·invoices·dashboard·sessions). **패턴**=형제 모듈 직접 require(무순환 대다수), 상호의존(invoices↔sessions: invoices→sessionRateAmount / sessions→isSessionInvoiced)만 함수 내부 지연 `require("../data")`로 순환 회피; 내부전용 헬퍼(normalizeTaskTypeDb·getManagerByUserId·nextInvoiceNumber·computeInvoiceDraft·resolveTaskEngineer·sessionFields 등) 공개 미노출. **공개 export 124개 분리 전후 완전 동일**(매 커밋 HEAD 대조·added/removed 0), 도메인별 개별 커밋 + 40개 테스트 + 머니패스 스모크(VAT 110만/10만·채번 INV-·1Pro 30만/3Pro 90만·세션 겹침·청구 잠금 양방향). **DEV_LOGIN 브라우저 E2E 검증**(치프·임시 DB, 실데이터/코드 무변경): 세션 목록·**생성**(운영시간 그리드→새 세션 추가 성공), 청구 목록/통계·인라인 상세·**입금 '완납 처리' 쓰기**(미수금 ₩250만→0·입금완료·payStatusOf/balanceOf 재파생), 대시보드 교차모듈(dashboardStats→invoiceStats·upcomingSessions), 세션↔청구 상호의존(invoiced 플래그) — **읽기·쓰기·교차모듈 무회귀 확인**. (TODO 9 완료)
- **그 이전 세션 — v1.0 릴리스 위생 점검(/audit Top5) + 모바일 반응형**: **버전 정정** `package.json` 0.1.0→**1.0.0**·description 현행화(3역할 내부도구). **CI 파이프라인 신설**(위 품질 참조 — `checkout@v5`·`setup-node@v5`, `node --test test/*.test.js`). **죽은 코드 제거**(청구 상세·인라인의 미사용 `issued` + 2축 분리 후 죽은 `status==="입금완료"` 비교, `invoices.routes`/`views.invoices`). **세션 겹침 검사에 '취소' 예외**(`assertNoSessionConflict` — 취소 세션은 룸 미점유, 점유 슬롯에도 기록 가능·회귀 테스트 2건). **data.js 모듈 분리 착수**(스튜디오 설정 → `src/data/studio.js`, `cleanTime`→`lib/date` 공유; TODO 9 참조). **모바일 목록 찌그러짐 전수 개선**(함정 17): `.badge` `whitespace-nowrap`, 청구·클라이언트·연락처 목록 행을 '제목 전폭 → 배지 줄 → 메타'로 재구성(375/390px 검증). **긴 안내문 전수 접기**: `explain()` 헬퍼 신설, 설정 항목 설명·폼 안내 등 긴 설명을 기본 접힘 '설명' 토글로(상태·오류·모달 경고는 유지).
- **그보다 이전 세션 — 도메인 확장·연동·정합**: 연락처 표시명 제거·전화 010-####-#### 정규화·치프 역할 전환(본인 잠금·최소 1치프); 청구서 할인(정액+정률·공급가 차감 후 VAT 재계산·거래명세서 할인 라인); 외주/하우스 담당자↔연락처 양방향 연동; 클라이언트 첨부 서류(매직바이트·인증 다운로드); Google People API 양방향 동기화(수동 버튼·삭제 양방향·fail-safe); 세션·담당자·매출 개선(담당 디렉터·룸 기본 A·예약담당자 기본값·외주단가 조건부·매출 메뉴); 프로젝트 폼 개선(마감일 제거·고객측 담당자·목록 외 이름→새 연락처); /audit 진단 후속(수동 청구 VAT 역산·세션 겹침 전 타입·금전 테스트 24개); 전방위 점검(ultrawork); 청구처 청구시점 이동·용어 통일·프로젝트 유형 폐기·연락처(담당자) 도메인 신설.
