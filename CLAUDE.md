...
5. (선택·소소) 곡·콘텐츠 곡별 **인라인 아티스트 편집**(`trackEditMenu`)은 아직 datalist(`dl-artists`) 자동완성 — 추가 폼의 artistCombo(검색+새 등록 모달)와 패턴 불일치. 통일하려면 per-row artistCombo화 필요(옵션 JSON·hidden cid·모달). 현재는 datalist 렌더 복구로 자동완성 자체는 동작(2026-07-03 수정).
