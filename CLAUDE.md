...
- **곡·콘텐츠 작업을 진행 단계 중심으로(2026-06-29)**: dense select 6개 폼(`taskCreateForm`)을 없애고 **진행 단계
  빠른 버튼**(`taskQuickAdd`: 보컬녹음·악기녹음=시간제, 보컬튠·믹싱·마스터링=고정, +기타는 그룹 optgroup)으로
  한 번에 작업 추가(기본 상태=대기). 곡 카드에 **진행 요약**(`trackProgressSummary`: 단계 그룹별 최신 상태 →
  "녹음 완료 · 후반 진행중 · 믹스·마스터 대기")을 한 줄로. `taskRow`는 컴팩트(종류·담당·상태·금액)로 줄이고
  단가·과금·수량은 행의 '편집' 상세로 이동(청구 모델은 불변). 톤=예약 폼의 버튼/그룹/단계적 노출 연속.
- **프로젝트 상세 탭 + 곡·콘텐츠 후반작업화(2026-06-29)**: ① 프로젝트 상세를 **탭**(세션 일정/곡·콘텐츠/자료 전달/
  청구, 청구는 청구권자만)으로. CSP 안전 URL 탭(`?tab=`), 메타 카드는 탭 위 고정, 활성 탭만 lazy 조회. 기본 탭=
  녹음→세션, 믹스→곡·콘텐츠. 액션 리다이렉트도 같은 탭 복귀(sessions/tracks/deliverables). `sessionsSection`에
  `expand`(탭 안에서 믹스도 펼침). ② **곡·콘텐츠 = 녹음과 별개의 후반작업**: 빠른 버튼을 보컬튠·오디오편집·믹싱·
  마스터링으로(녹음 보컬/악기는 세션 흐름). +기타 드롭다운엔 전체 종류 유지.
- **청구 대기 목록(2026-06-29)**: 청구 탭에 **'청구 대기 · 완료된 녹음 세션'** 블록 추가(`pendingSessionsForm`) —
  완료(status=완료)+단가+시간으로 산정된 녹음 세션 중 아직 청구 작업으로 전환 안 된 것(`billed_task_id` null)을
  예상 청구액과 함께 노출, **'청구 확정'** 버튼(`POST /sessions/:id/bill`, 트랙 자동 생성)으로 track_task 전환 →
  기존 '미청구 작업' 폼으로 합류. 확정 후 `?tab=invoice` 복귀. **완료된 곡·콘텐츠 작업은 이미 '미청구 작업'에
  노출**(별도 작업 불필요). 부팅 통합(완료 세션→청구 대기·확정→미청구 전환) 통과. **남은 TODO: 작업 종류
  정식 관리(환경설정 — 단가표처럼, 사용자 승인됨).**
