...
### 6-0. 테스트 체계 — 3층 방어선 (`npm test`, 118개, CI Node 20/22 동일 실행)
...
| 층 | 파일 | 개수 | 검증 대상 |
|---|---|---|---|
| **① 단위** | `invoice-number`·`vat`·`rate-price`·`session-conflict`·`auth`·`party`·`payments`·`rental-session` | 77 | 금전 로직(채번·VAT·Pro블록 단가·할인)·세션 겹침(야간교차·취소예외)·권한·당사자 모델·입금 이력·대관 세션(녹음/촬영/공연 kind 매핑·금액 미정 정액) |
