...
> 곡·콘텐츠(후반작업·**금액은 청구 탭에서 확정**[작업 폼=종류·담당·상태만, 청구 생성 폼에서 작업별 금액 입력·종류 기본단가 자동]·외주 지급단가 `worker_rate`·`engineer_id`) + 작업 종류 카탈로그(DB 관리·삭제-only) + **거래명세서 PDF**(resvg + 한글 폰트 번들·문서명 3종) +
