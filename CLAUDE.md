...
- **이번 세션 — v1.0 릴리스 위생 점검(/audit Top5) + 모바일 반응형**: **버전 정정** `package.json` 0.1.0→**1.0.0**·description 현행화(3역할 내부도구). **CI 파이프라인 신설**(위 품질 참조 — `checkout@v5`·`setup-node@v5`, `node --test test/*.test.js`). **죽은 코드 제거**(청구 상세·인라인의 미사용 `issued` + 2축 분리 후 죽은 `status==="입금완료"` 비교, `invoices.routes`/`views.invoices`). **세션 겹침 검사에 '취소' 예외**(`assertNoSessionConflict` — 취소 세션은 룸 미점유, 점유 슬롯에도 기록 가능·회귀 테스트 2건). **data.js 모듈 분리 착수**(스튜디오 설정 → `src/data/studio.js`, `cleanTime`→`lib/date` 공유; TODO 9 참조). **모바일 목록 찌그러짐 전수 개선**(함정 17): `.badge` `whitespace-nowrap`, 청구·클라이언트·연락처 목록 행을 '제목 전폭 → 배지 줄 → 메타'로 재구성(375/390px 검증).
- **직전 세션 — 도메인 확장·연동·정합**: 연락처 표시명 제거·전화 010-####-#### 정규화·치프 역할 전환(본인 잠금·최소 1치프); 청구서 할인(정액+정률·공급가 차감 후 VAT 재계산·거래명세서 할인 라인); 외주/하우스 담당자↔연락처 양방향 연동; 클라이언트 첨부 서류(매직바이트·인증 다운로드); Google People API 양방향 동기화(수동 버튼·삭제 양방향·fail-safe); 세션·담당자·매출 개선(담당 디렉터·룸 기본 A·예약담당자 기본값·외주단가 조건부·매출 메뉴); 프로젝트 폼 개선(마감일 제거·고객측 담당자·목록 외 이름→새 연락처); /audit 진단 후속(수동 청구 VAT 역산·세션 겹침 전 타입·금전 테스트 24개); 전방위 점검(ultrawork); 청구처 청구시점 이동·용어 통일·프로젝트 유형 폐기·연락처(담당자) 도메인 신설.
- **그 이전 세션 — 디자인·백엔드 기반**: 디자인 기반(Pretendard·쿨톤 info색·사이드바 그룹화·테마 토글·`listGroup`/`listRow`/`emptyState`·opacity.12); 백엔드 정리(`resolveEndTime`·0원 가드·죽은코드·`parseMoney`/`timeToMin` 통합·운영시간 인프라); 라운드2 UX(세션폼 조건부 단가·예정 완료1클릭·검색·운영시간 슬롯·청구 진입점 단일화·클라이언트 검색·대시보드 강화).
