...
> **완료(이번 세션·2026-07-14 최신)**: **날짜 콤보 앱 전면 적용**(사용자 요청) — `views.dateCombo` 공용 헬퍼 + app.js 전역 초기화(`__initDateCombos`). 세션(시작·종료)·프로젝트 작성일(자동제출 유지)·청구 발행일·자료 만료일(행·업로드)·외주 지급일·소속 이력(시작·종료) 8곳. 네이티브 `type=date` 0개. 274 테스트·E2E.
