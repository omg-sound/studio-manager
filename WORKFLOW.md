...
7. (진행, L) **data.js 모듈 분리** — 2049→1682줄. 재export 허브(공개 export 124개 분리 전후 동일·자동 대조 검증). 완료: studio·client-files·revenue·deliverables·rooms·rate-items·task-types(모듈 캐시 포함). 남은 코어(프로젝트·작업·청구·세션·클라이언트·연락처)는 상호결합 강해 사용자 참여하 신중 이관.
8. (보류) **content_type/billing_type UI 노출** — `content_type[Music|Video_Post]`·`billing_type` 현재 미노출/강제; 영상 구분·과금 유형 선택은 향후 확장 시 복원.

> **완료(이번 세션)**: **data.js 모듈 분리 2차**(TODO 7) — `data.js` 2049→1682줄, 6개 도메인 추가 추출: `client-files.js`(첨부 서류)·`revenue.js`(매출)·`deliverables.js`(자료 전달)·`rooms.js`(룸)·`rate-items.js`(단가표·computeRatePrice)·`task-types.js`(작업 종류 카탈로그·**모듈 캐시 포함**). 재export 허브 패턴 유지, **공개 export 124개 분리 전후 완전 동일**(HEAD~4 원본과 자동 대조·added/removed 0). cross-domain은 로컬 바인딩(computeRatePrice·listRooms) 또는 함수 내부 지연 require(revenue·deliverables·순환 회피)로 해소, 내부전용 normalizeTaskTypeDb는 로컬 바인딩만(공개 미노출). 고아 import 정리(crypto·normalizeBillingType·normalizeRecordingCategory). 각 도메인마다 40개 테스트 + 스모크(computeRatePrice 3Pro=90만·task_types 캐시 무효화·지연 require 실호출) 통과, 도메인별 개별 커밋·push. 남은 코어(프로젝트·작업·청구·세션·클라이언트·연락처)는 상호결합 강해 사용자 참여하 신중 이관.
