...
- **상세에 담당자 연락처**
- **클라이언트 상세 = 인라인 편집**(2026-07-01): '정보 수정' 버튼 폐기 → 상세 상단에 `clientForm` 인라인(dirty 저장·첨부 서류·삭제 포함, `embedded`로 자체 pageHeader 생략). `/:id/edit`는 상세로 리다이렉트(첨부 `?ferr=` 보존), 첨부 업로드/삭제도 상세로 복귀. 파생 링크(연동 연락처·대표자 연락처)+소속 아티스트·담당자 연락처 목록·프로젝트/청구 탭은 폼 아래 유지. 폼 대표자/담당자 datalist는 전체 연락처.: 회사(client) 상세 상단에 그 회사의 **현재 소속 연락처(직원)** 목록(`listContactsForClient`, `ended_on IS NULL`) 노출.
