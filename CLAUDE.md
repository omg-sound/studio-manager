...
- 🏗 **당사자(Party) 모델 — 정체성 통합**(계획: `~/.claude/plans/temporal-chasing-lighthouse.md`). clients/contacts 정체성 이중화(source_contact_id 셸·'기타'·is_group)를 근본 제거하고 **`parties` 한 테이블**로 통합(사용자 확정). 역할(아티스트·청구처·담당자·디렉터·엔지니어)은 테이블이 아니라 `parties.id` 참조. kind=person|company|group, is_artist=아티스트 역할 플래그.
...
- `parties(id, kind[person|company|group], name, activity_name?[활동명], is_artist, phone?, email?, memo?, / person: family_name·given_name·honorific·department·job_title·user_id·google_resource_name·google_etag·cash_receipt_no / company: biz_no·owner_name·owner_party_id·address·roles)` — **당사자 통합 마스터**(이관 중, P2부터 읽기). `affiliations(person_id→parties CASCADE, org_id→parties SET NULL, title?, started_on?, ended_on?, memo?)` — 소속 이력(contact_affiliations 이관본).
