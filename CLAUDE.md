...
- **이번 세션**: **입력 콤보 전면 통일** — `personCombo`(사람 검색+새 등록 모달)·`payerCombo`(청구처, 클라/담당자 검색)를 `views.js` 공용화 + app.js `window.__initPersonCombos`(동적 디렉터 행 초기화). 프로젝트 담당자·클라이언트 담당자·세션 디렉터·청구처가 전부 동일 검색+모달 콤보(선택 시 닫힘). 아티스트 콤보=본명 입력칸 안 오버레이·본명 검색. 옛 datalist(`[data-client/contact-combo]`)·`syncCombo`·`clientCombo` 제거. **Drive 자료 저장을 studio@omgworks.kr로 영구 고정**(`config.studioDriveEmail`, 그 계정 로그인 시에만 refresh token 저장 — 치프 무관·고정; `/auth/google?drive=1`=계정 선택기 강제). **첨부 저장 안전화**(Drive 연결 상태 업로드 실패 시 로컬 폴백 대신 예외 `DRIVE_UPLOAD_FAILED`, 저장 위치 배지). 연락처 목록=클라이언트 목록 통일(이름·회사 각각 링크). **전화·이메일·주소·사업자번호 클릭 복사**(`copyable`+토스트, tel/mailto 대체, 아이콘 없이 hover 밑줄). 사업자번호=복사·등록증 보기 분리. **소속사/제작사도 콤보+간이 등록 모달**(companyCombo)·**고객측 담당자도 personCombo**(모달 회사칸=기존 클라이언트 datalist autocomplete). **리포트 후속(F1/I1/I2/I3)**: contacts.routes async 핸들러 asyncHandler 래핑(People API throw→요청 행 방지); **일일 DB 백업 Drive 오프사이트**(`drive.backupToDrive`, keep 14, fail-safe); **청구처 정보 발행 시점 스냅샷**(`invoices.payer_snapshot`·`payerView`/`payerName`, 발행 후 클라이언트 정보 변경돼도 청구서/PDF 고정); **콤보 옵션 JSON 중복 임베드 제거**(디렉터=폼당 공유 스크립트 `optionsRef`/`personComboOptionsScript`, 페이지 축소). **클라이언트 상세 '연결 프로젝트/청구' 버그 수정**(listProjectsForParty에 party 객체 대신 c.id — Number(obj)=NaN이라 0건). **청구 생성 폼 청구처 유형 UX**: 회사=버튼 '(계산서 발행)' / 개인='(현금영수증 발행)' 동적 라벨(app.js picker `applyDoc`), **발행 정보 없으면 경고+청구 생성 차단**(회사=세금계산서 정보 biz_no, 개인=현금영수증 cash_receipt_no; 클라 preventDefault+서버 `payerDocMissing` throw). **미완료(대기·진행중) 콘텐츠 청구 시 확인**('완료로 바꿀까?')·**청구 생성 시 선택 작업 status='Completed' 전환**(청구=완료). 유지보수 점검: 전 모듈 로드·55 테스트·빌드·전 페이지 21개 200·쓰기 스모크 무에러·프로덕션 배포 확인.
