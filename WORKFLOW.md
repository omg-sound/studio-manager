...
> 알림 채널(웹훅·암호화·fail-safe) + 관리 항목 삭제 + 녹음 세션 직접 청구(청구 탭 자동 노출·세션 잠금·**청구처는 청구 시점 결정**·발행후 변경 잠금·**인보이스 인라인 펼침·생성**[생성('선택 항목으로 청구 생성'·수동)·상세·입금·상태·PDF·삭제 전부 청구 탭에서 — 청구 메뉴 이탈 없이 프로젝트 내 완결, `return` 복귀·`?open=` 펼침; 청구 메뉴=모아보기]·**청구서 수정 폐기**[발행=확정, 변경은 삭제 후 재발행]) +
