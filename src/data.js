...
const { getProjectForUser } = projects; // 내부 호출 유지용(tracks·sessions·invoices 다수 잔여 도메인)
const clientsMod = require("./data/clients"); // 클라이언트(거래처)+담당자 마스터 도메인(분리 모듈) — 아래에서 재export
const { getManagerByUserId, ...clientsPublic } = clientsMod; // getManagerByUserId=내부전용(tracks createTask, 공개 미노출), 나머지=공개 재export
...
module.exports = {
  ...
  updateTaskType,
  deleteTaskType,
  ...projects, // 프로젝트 도메인 재export(src/data/projects.js): distinctProjectFields·listProjects·getProjectForUser
  deleteProject,
