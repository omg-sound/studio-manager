...
- 🚧 **당사자(Party) 모델 이관 진행 중**(계획: `~/.claude/plans/temporal-chasing-lighthouse.md`) — clients/contacts 정체성 이중화(source_contact_id 셸·'기타'·is_group)를 근본 제거하려 **`parties` 한 테이블**로 통합 중(사용자 확정). 역할(아티스트·청구처·담당자·디렉터·엔지니어)은 테이블이 아니라 `parties.id` 참조. **P1(완료)**: `parties`·`affiliations` 테이블 + 역할 컬럼(`invoices.payer_id`·`projects.artist_id/agency_id/production_id/contact_party_id`·`project_managers.party_id`·`sessions.director_party_id`·`session_directors.party_id`) **추가 + `party_model_v1` 1회 이관**(contacts→person, 소속사/제작사→company, 그룹 아티스트→group, 솔로 아티스트·'기타' 셸→해당 사람 party 병합). **P1은 순수 populate — 읽기·쓰기 경로는 아직 clients/contacts 사용**(휴면 컬럼, P2에서 전환·P4에서 레거시 제거). db.js `migrateToPartyModel` + 이관 리포트 로그(orphan_payer=0 정상). 아래 clients/contacts 스키마는 **레거시(이관 원본)**.
