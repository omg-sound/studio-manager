...
test("theme-init.js: data-theme·data-palette 조기 적용(FOUC 방지) + 기본 팔레트 Linear·테마 OS 추종", () => {
  ...
  // 테마(라이트/다크)는 OS 추종 — 저장값 없으면 data-theme 미설정(강제 안 함, 2026-07-18).
  assert.match(init, /if \(t === "dark" \|\| t === "light"\) document\.documentElement\.setAttribute\("data-theme"/, "테마는 저장값 있을 때만 설정(OS 추종)");
