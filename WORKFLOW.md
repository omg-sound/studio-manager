...
### 6-0. 테스트 체계 — 3층 방어선 + 스모크 (`npm test`, 255개, CI Node 20/22 동일 실행)
...
```bash
npm test                                   # 전체 255개(단위+가드+상호작용+스모크·카카오 라우트 보안)
...
> **완료(이번 세션·2026-07-14 최신)**: **프로젝트 목록 지메일식 한 줄 행 + 밀도 토글(좁게/넓게)** — 행=`<details>`(행 클릭=펼침·제목만 상세), 좁게 기본(아티스트/제작사·프로젝트명/탭별 우측값·42px), 넓게=동일 DOM에 CSS 전환(`:root[data-density="comfy"]`·작성일·PM·카운트 요약·구분선 목록), 토글=localStorage(서버 왕복 0·FOUC 없음). `src/views.projects.js`·`public/css/src.css`·`public/js/app.js`·`src/routes/projects.routes.js`. 255 테스트·실브라우저 E2E(클릭 규약 2종·두 밀도·모바일 390px 오버플로우 0). 설계=docs/superpowers/specs/2026-07-14-project-list-gmail-density-design.md.
