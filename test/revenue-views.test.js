...
test("revStaffDetail: 4칸 행 — 발행일(일만)·종류·프로젝트·세부·금액", () => {
  ...
  assert.match(html, /rev-item-kind[^>]*>[\s\S]*?mixing/, "작업은 작업 종류");
  assert.match(html, /rev-item-kind[^>]*>[\s\S]*?공연/, "세션은 세션 종류");
...
test("revStaffDetail: 곡 제목이 프로젝트명과 같으면 세부를 생략한다", () => {
  ...
  const html = V.revStaffDetail(data);
  // 의도 = **보이는 행**에 같은 이름이 두 번 안 나오는 것. (title 속성은 잘림 대비 전체 텍스트를 담으므로 세지 않는다.)
  assert.ok(!/· Inferno/.test(html), "세부(· 곡제목) 자체가 렌더되지 않는다");
  assert.match(html, /font-medium">Inferno<\/span> ↗/, "프로젝트명은 그대로");
});

test("revPayerDetail: 청구서 링크는 새 탭(target=_blank rel=noopener) + 이름 끝에 ↗", () => {
  const data = {
    party: { id: 5, name: "도너츠컬처", kind: "company" },
...
test("revPayerDetail: 프로젝트가 없는 청구서는 '청구 #id'로 표시", () => {
  const html = V.revPayerDetail({
    party: { id: 5, name: "회사", kind: "company" },
    invoices: [{ id: 77, invoice_number: "OMG-202607-001", issued_date: "2026-07-01", amount: 1100, tax_amount: 100, supply: 1000, tax_status: "입금완료", status: "발행", payer_kind: "company", project_title: null }],
    supply: 1000, invoice_cnt: 1,
  });
  assert.match(html, /청구 #77/, "프로젝트 없으면 청구 번호로 대체");
});
...
// 2026-07-20 사용자 요청: 행의 이름을 프로젝트명 → **아티스트명**으로
// (프로젝트명이 '회사명 + 아티스트'를 반복해 길고 덜 정확했다 — 실측 프로덕션 92/92가 아티스트 보유).
test("revStaffDetail: 이름은 아티스트, 없으면 프로젝트명 폴백", () => {
  const base = { manager: { id: 3, name: "김엔지", user_id: 1 }, supply: 1, payout: 0, profit: 1 };
  const withArtist = V.revStaffDetail({ ...base,
    tasks: [{ id: 1, task_type: "Mixing", amount: 1, worker_rate: 0, track_title: "가끔", project_id: 9, project_title: "더블유엠코퍼레이션 차가을", artist: "차가을", issued_date: "2026-07-16" }],
    sessions: [] });
  assert.match(withArtist, /font-medium">차가을</, "아티스트명이 이름");
  assert.ok(!/더블유엠코퍼레이션/.test(withArtist), "긴 프로젝트명은 안 쓴다");

  const noArtist = V.revStaffDetail({ ...base,
    tasks: [{ id: 1, task_type: "Mixing", amount: 1, worker_rate: 0, track_title: "가끔", project_id: 9, project_title: "프로젝트명만", artist: "", issued_date: "2026-07-16" }],
    sessions: [] });
  assert.match(noArtist, /font-medium">프로젝트명만</, "아티스트 없으면 프로젝트명");
});

test("revStaffDetail: 아티스트가 여럿이면 '첫 외 N'(프로젝트 목록과 같은 규칙)", () => {
  const html = V.revStaffDetail({ manager: { id: 3, name: "김엔지", user_id: 1 }, supply: 1, payout: 0, profit: 1,
    tasks: [{ id: 1, task_type: "Mixing", amount: 1, worker_rate: 0, track_title: "곡", project_id: 9, project_title: "P", artist: "추화정, 선진, 박연", issued_date: "2026-07-16" }],
    sessions: [] });
  assert.match(html, /추화정 외 2/, "다중 아티스트 축약");
});

// 이름이 아티스트로 바뀐 뒤의 함정: 곡 제목 중복 판정을 **표시 이름**이 아니라 프로젝트명과 비교하면,
// 화면에 없는 값과 같다는 이유로 필요한 곡 제목이 사라진다.
test("revStaffDetail: 곡 제목은 표시 이름과 같을 때만 생략(프로젝트명과 같아도 남긴다)", () => {
  const base = { manager: { id: 3, name: "김엔지", user_id: 1 }, supply: 1, payout: 0, profit: 1, sessions: [] };
  // 프로젝트명 == 곡 제목이지만 이름은 아티스트(재호) → 곡 제목은 살아야 한다
  const keep = V.revStaffDetail({ ...base,
    tasks: [{ id: 1, task_type: "Mixing", amount: 1, worker_rate: 0, track_title: "Inferno", project_id: 9, project_title: "Inferno", artist: "재호", issued_date: "2026-07-16" }] });
  assert.match(keep, /font-medium">재호<\/span> <span class="text-muted">· Inferno/, "곡 제목 유지");
  // 아티스트 == 곡 제목이면 같은 말 두 번 → 생략
  const drop = V.revStaffDetail({ ...base,
    tasks: [{ id: 1, task_type: "Mixing", amount: 1, worker_rate: 0, track_title: "재호", project_id: 9, project_title: "P", artist: "재호", issued_date: "2026-07-16" }] });
  assert.ok(!/· 재호/.test(drop), "표시 이름과 같으면 생략");
});

test("revPayerDetail: 이름은 아티스트(폴백·중복 규칙 동일)", () => {
  const mk = (artist, project_title, work_detail) => V.revPayerDetail({
    party: { id: 5, name: "회사", kind: "company" },
    invoices: [{ id: 1, issued_date: "2026-07-16", amount: 110000, tax_amount: 10000, supply: 100000, tax_status: "입금완료", status: "발행", payer_kind: "company", project_title, artist, work_kind: "믹싱", work_detail, item_count: 1 }],
    supply: 100000, invoice_cnt: 1,
  });
  assert.match(mk("임단우", "도너츠컬처 임단우", ""), /font-medium">임단우</, "아티스트명이 이름");
...
// ── 미귀속 행(2026-07-20) ────────────────────────────────────────────────────
test("revStaffList: 미귀속은 순위에 끼지 않고 맨 아래 + 전용 링크", () => {
  const rows = [
    { id: 3, name: "김엔지", is_external: false, supply: 200000, profit: 150000, task_cnt: 1, session_cnt: 0 },
    { id: 4, name: "이엔지", is_external: true, supply: 100000, profit: 90000, task_cnt: 1, session_cnt: 0 },
  ];
  const html = V.revStaffList(rows, { unattributed: { supply: 650000, profit: 650000, task_cnt: 0, session_cnt: 2 } });
  assert.match(html, /href="\/revenue\?tab=staff&amp;staff=none"/, "미귀속 전용 링크");
  assert.match(html, /₩650,000/);
  assert.match(html, /작업 0 · 세션 2/);
  // 매출이 제일 큰데도 맨 아래여야 한다 — 사람이 아니라서 순위에 섞으면 '1위'가 사람이 아니게 된다.
  assert.ok(html.indexOf("staff=none") > html.indexOf("staff=4"), "매출이 커도 맨 아래");
});

test("revStaffList: 미귀속이 0원이면 행 자체가 없다", () => {
  const html = V.revStaffList([{ id: 3, name: "김엔지", is_external: false, supply: 200000, profit: 150000, task_cnt: 1, session_cnt: 0 }],
    { unattributed: { supply: 0, profit: 0, task_cnt: 0, session_cnt: 0 } });
  assert.ok(!html.includes("staff=none"), "0이면 안 보인다(평소엔 없는 줄)");
});

test("revStaffList: 스탭이 없어도 미귀속만 있으면 목록을 낸다", () => {
  const html = V.revStaffList([], { unattributed: { supply: 50000, profit: 50000, task_cnt: 1, session_cnt: 0 } });
  assert.match(html, /staff=none/, "빈 상태 문구로 미귀속을 덮지 않는다");
});

test("revStaffList: 미귀속 선택 시 그 행만 aria-current", () => {
  const html = V.revStaffList([{ id: 3, name: "김엔지", is_external: false, supply: 200000, profit: 150000, task_cnt: 1, session_cnt: 0 }],
    { selId: "none", unattributed: { supply: 650000, profit: 650000, task_cnt: 0, session_cnt: 2 } });
  assert.equal((html.match(/aria-current="page"/g) || []).length, 1);
  assert.match(html, /staff=none"[^>]*aria-current="page"/);
});
