...
> **완료(이번 세션·2026-07-11h 최신)**: **완료 UI 일관성 정리**(개선점 찾기에서 선택) — 네 완료 표면 중 캘린더 팝오버만 '완료 처리'/'✓ 완료됨'(btn-primary)로 달랐던 것을 나머지와 동일한 `− 완료`/`✓ 완료` glyph 토글로 통일(모달이라 btn-sm 크기만). `sessionCardModal` 버튼만. 199 테스트·실브라우저 확인.
