...
test("0원 항목: confirmZero 없으면 TASK_AMOUNT_REQUIRED로 차단", () => {
  ...
  assert.throws(() => createInvoiceFromTasks(CHIEF, { projectId, taskIds: [taskId], issueDate: "2026-07-05" }), /TASK_AMOUNT_REQUIRED/);
});

test("0원 항목: confirmZero:true면 0원 그대로 청구 생성(공급가·총액 0)", () => {
  const { projectId, taskId } = seedTask(0);
  const inv = createInvoiceFromTasks(CHIEF, { projectId, taskIds: [taskId], issueDate: "2026-07-05", confirmZero: true });
...
test("청구 항목 정렬: 세션(이전 날짜)+작업(이후 생성일) 혼합 시 item_date 기준 날짜순", () => {
  ...
  const inv = createInvoiceFromTasks(CHIEF, { projectId: pid, taskIds: [taskId], sessionIds: [sessionId], issueDate: "2026-08-05" });
  assert.ok(inv);
