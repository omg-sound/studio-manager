...
- 🏗 **당사자(Party) 모델 — 정체성 통합**(계획: `~/.claude/plans/temporal-chasing-lighthouse.md`). clients/contacts 정체성 이중화(source_contact_id 셸·'기타'·is_group)를 근본 제거하고 **`parties` 한 테이블**로 통합(사용자 확정). 역할(아티스트·청구처·담당자·디렉터·엔지니어)은 테이블이 아니라 `parties.id` 참조. kind=person|company|group, is_artist=아티스트 역할 플래그.
  ...
  - **P4 안전 정리(완료)**: 죽은 모듈 `src/data/contacts.js`·`clients.js` 삭제(data.js가 걷어내 무참조), `linkClientContact` 소속 중복체크를 `affiliations`로 교정, `seed.js`를 parties로(회사·아티스트·프로젝트 party·payer). **보수적 유지(라이브 마이그레이션 직후 안전망)**: 레거시 테이블(clients·contacts·contact_affiliations)·컬럼(source_contact_id·is_group·'기타'·invoices.client_id·projects.client_id/contact_id)·`compat.js`·이관 게이트·③④⑤ 백필은 **존치** — 프로덕션 안정 확인 후 별도 커밋에서 드롭. **`projects.artist/artist_company/production_company` TEXT는 표시 denormalization으로 유지(목록·요약 렌더)**.
  - 아래 clients/contacts 스키마는 **레거시(이관 원본, 이후 드롭 대상)**.
