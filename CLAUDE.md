...
- **분류 3분리 + 유형 먼저 선택 흐름**(2026-07-02): 클라이언트 목록 탭 = **전체 / 업체 / 아티스트(개인 솔로) / 그룹(밴드·아이돌)** 4분류(상호배타, `isSoloArtist`=person·is_artist / 그룹=kind='group' / 업체=company). '+ 새 클라이언트'는 **유형 3택 진입**(`GET /clients/new` → ?type 없으면 업체/아티스트/그룹 3카드, 있으면 유형별 폼) — 서로 섞이지 않는 3개념, **'기타' 폐기**. `clientForm(formType)`: **kind select 제거·유형 고정**(편집 시 유형 배지+'변경 불가', 유형은 party.kind라 불변). 업체=역할 체크·사업자·대표·주소·담당자 콤보·첨부, 아티스트=현금영수증·**소속 그룹 select**, 그룹=그룹명·현금영수증(멤버는 상세). `POST /`는 `type`(hidden)으로 `createCompany/createPerson[artist]/createGroup` 디스패치. **업체 역할 표기 '제작사'→'제작사/운영사'**(`companyRoleLabel` — 저장값은 `제작사` 유지, 프로젝트 파생·마이그레이션 호환; 폼 체크박스·목록/상세 배지 표시만 변경). app.js `[data-client-kind]` 토글 IIFE는 유형 고정으로 미사용(무해 no-op). `companyRolesFrom(b)` 단순화(체크된 roles CSV 또는 null).
