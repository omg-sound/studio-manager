...
- **이번 세션**: **입력 콤보 전면 통일** — `personCombo`(사람 검색+새 등록 모달)·`payerCombo`(청구처, 클라/담당자 검색)를 `views.js` 공용화 + app.js `window.__initPersonCombos`(동적 디렉터 행 초기화). 프로젝트 담당자·클라이언트 담당자·세션 디렉터·청구처가 전부 동일 검색+모달 콤보(선택 시 닫힘). 아티스트 콤보=본명 입력칸 안 오버레이·본명 검색. 옛 datalist(`[data-client/contact-combo]`)·`syncCombo`·`clientCombo` 제거. **Drive 자료 저장을 studio@omgworks.kr로 영구 고정**(`config.studioDriveEmail`, 그 계정 로그인 시에만 refresh token 저장 — 치프 무관·고정; `/auth/google?drive=1`=계정 선택기 강제). **첨부 저장 안전화**(Drive 연결 상태 업로드 실패 시 로컬 폴백 대신 예외 `DRIVE_UPLOAD_FAILED`, 저장 위치 배지). 연락처 목록=클라이언트 목록 통일(이름·회사 각각 링크). **전화·이메일·주소·사업자번호 클릭 복사**(`copyable`+토스트, tel/mailto 대체). 사업자번호=복사·등록증 보기 분리.
