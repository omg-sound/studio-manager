...
// ── 요약 카드(미수금·이번 달·올해) 반응형(2026-07-20 메인터넌스 실측) ──
// 사고: 사이드바가 나타나는 sm(640)부터 3열이 되는데, 그 구간 본문은 ~396px뿐이라 카드 한 칸이 124px로
// 쪼그라들고 금액(₩6,800,000 = 109px)이 카드 밖으로 삐져나갔다 → 640px에서 가로 넘침 47px.
// 3열은 본문이 충분히 넓어지는 lg부터. 라벨은 줄이고 금액은 안 줄인다(금액이 잘리면 못 읽는다).
test("청구 요약 카드: 3열은 lg부터(sm~md 구간은 세로) + 금액은 안 줄어든다", () => {
  const src = require("fs").readFileSync(require("path").join(__dirname, "..", "src", "routes", "invoices.routes.js"), "utf8");
  assert.match(src, /class="mb-4 grid gap-3 lg:grid-cols-3"/, "3열은 lg부터");
  assert.ok(!/grid gap-3 sm:grid-cols-3/.test(src), "옛 sm 3열 잔존 없음(640~1023에서 금액이 삐져나갔다)");
  assert.match(src, /class="tabular shrink-0 text-lg font-bold/, "금액은 shrink-0 — 좁아져도 안 줄인다");
  assert.match(src, /class="min-w-0 text-sm text-muted"/, "라벨이 대신 줄어든다");
});
