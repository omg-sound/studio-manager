...
> 곡·콘텐츠(후반작업·**금액은 청구 탭에서 확정**[작업 폼=종류·담당·상태만, 청구 생성 폼에서 작업별 금액 입력·종류 기본단가 자동]·외주 지급단가 `worker_rate`·`engineer_id`) + 작업 종류 카탈로그(DB 관리·삭제-only·**분류 폐기**[모두 후반작업]) + **거래명세서 PDF**(resvg + 한글 폰트 번들·문서명 3종) +
> 알림 채널(웹훅·암호화·fail-safe) + 관리 항목 삭제 + 녹음 세션 직접 청구(청구 탭 자동 노출·세션 잠금·**청구처는 청구 시점 결정**·발행후 변경 잠금·**인보이스 인라인 펼침·생성·수정**[생성('선택 항목으로 청구 생성'·수동)·상세·입금·상태·PDF·**수정(펼친 자리 인라인 폼·invoiceForm embed)**·삭제 전부 청구 탭에서 — 청구 메뉴 이탈 없이 프로젝트 내 완결, `return` 복귀·`?open=` 펼침; 청구 메뉴=모아보기]) +
