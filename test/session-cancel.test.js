...
test("sessionCardModal: app.js가 좌표를 넣을 훅과 절대 위치 지정이 있다", () => {
  ...
  assert.match(html, /class="card absolute /, "좌표(left/top)가 먹으려면 absolute라야 한다");
  assert.match(html, /data-modal /, "스크롤 잠금·✕ 닫기 공용 처리는 그대로 탄다");
  assert.match(html, /fixed inset-0/, "바깥 레이어는 전체를 덮어 바깥 클릭 닫기를 받는다");
});
