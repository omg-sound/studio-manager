...
// ── ③d 편집 폼 디렉터 프리필(콤마 다중 콤보, 2026-07-05 행 UI 폐지): 기존 디렉터들이 라벨 콤마 목록으로 렌더 ──
test("세션 편집 폼: 기존 담당 디렉터가 콤마 콤보에 라벨 목록으로 채워짐", () => {
  ...
  assert.ok(combo, "디렉터 = 콤마 다중 personCombo 하나(행 UI 없음)");
  assert.equal(doc.querySelector("[data-director-row]"), null, "옛 행 마크업 제거");
  const visible = combo.querySelector("[data-pc-input]");
  const hidName = combo.querySelector("[data-pc-name-hidden]");
  assert.equal(visible.value, "감초아 (초아비트), 전범선 대표님", "라벨 콤마 목록 프리필(personName — created_at 동률 시 이름순)");
  assert.equal(hidName.value, "감초아 (초아비트), 전범선 대표님", "제출 = 전체 텍스트(서버가 라벨 안전망으로 해석)");
  assert.equal(combo.querySelector("[data-pc-id]").value, "", "multi는 hidden id 미사용");
  // 종일 세션이므로 시간 UI는 숨김, 종일 체크됨
  assert.equal(doc.querySelector("[data-all-day]").checked, true, "종일 세션 → 체크");
});
test("personCombo(multi): 콤마 뒤 조각 검색·선택 이어붙임·hidName=전체 텍스트", () => {
  const html = `<form>${personCombo({ options: PC_OPTS, companyOptions: [], multi: true, nameField: "director_name" })}</form>`;
  const { win, doc } = mountDom(html);
  const input = doc.querySelector("[data-pc-input]");
  const pop = doc.querySelector("[data-pc-pop]");
  const hidName = doc.querySelector("[data-pc-name-hidden]");
  // 첫 디렉터 선택 → 라벨
  input.value = "워터"; fire(win, input, "input");
  fire(win, pop.querySelector("button[data-idx]"), "click");
  assert.equal(input.value, "박수한 대표님 (워터멜론)", "첫 선택 = 라벨");
  // 콤마 후 둘째 검색 — 조각으로만
  input.value = "박수한 대표님 (워터멜론), 김보"; fire(win, input, "input");
  const row = pop.querySelector("button[data-idx]");
  assert.ok(row && row.textContent.includes("김보통"), "마지막 조각으로 둘째 검색");
  fire(win, row, "click");
  assert.equal(input.value, "박수한 대표님 (워터멜론), 김보통", "이어붙임");
  assert.equal(hidName.value, "박수한 대표님 (워터멜론), 김보통", "제출 = 전체 텍스트");
  assert.equal(doc.querySelector("[data-pc-id]").value, "", "id 미사용");
  // 새등록 행 라벨 = 마지막 조각
  input.value = "박수한 대표님 (워터멜론), 신인디렉터"; fire(win, input, "input");
  const nw = pop.querySelector("button[data-new]");
  assert.ok(nw && nw.textContent.includes("'신인디렉터'"), "새 등록 라벨 = 조각");
});
