...
- 예약 폼=버튼 UX: 시작 시간 그리드(**운영시간 기반 동적 생성** — `studioStartSlots`가 `admin_state.studio_hours` 읽음; 예약된 슬롯 회색, **선택=테두리 강조**). **그리드 맨 뒤 '직접입력' 셀 = 입력 박스 자체**(2026-07-04, 버튼 토글 폐기 — 박스에 바로 타이핑; 숫자만 입력하면 **콜론 자동 삽입 `1400`→`14:00`**, `pattern`+서버 `cleanTime` 검증; 직접입력 시 그리드 라디오 해제=직접입력 우선). placeholder='직접입력'. (이전의 '녹음 종류 미선택 시 시작 시간 비활성' 필수 게이트는 세션 종류 가변화로 **미사용** — 대신 세션 종류=녹음일 때만 녹음 단가 항목 select 노출로 대체. `rateSelectGrouped`의 `required`/`data-rate-required` 인터페이스 보존.)
- 소요시간 **슬라이더**(30분 단위·**최대 16시간**=`DURATION_SLIDER_MAX=960`·서버 `resolveEndTime` 960분 클램프, 2026-07-04 14h→16h) + 아래 `[1Pro][2Pro][3Pro][4Pro]` 프리셋(슬라이더와 양방향 동기화). **프리셋 배치**: 데스크톱(sm+)=슬라이더 위치에 절대배치(`positionTicks`가 기준시간으로 재배치·4Pro=base×4가 16h 트랙의 해당 %), **모바일(<640px)=위치정렬 없이 앞에서부터 흐름 정렬**(`sm:absolute`/`sm:left-*`·모바일은 `static`+flex, `positionTicks`가 모바일이면 인라인 위치 제거·resize 재계산, 사용자 요청). 종료는 서버가 시작+길이로 계산(`custom_hours`+`duration_mode=custom`, 1Pro=녹음 종류 기준시간).
...
- 프로젝트 하위 곡/콘텐츠(`project_tracks`) + 모듈형 작업(`track_tasks`). **진행 단계 빠른 버튼**(보컬튠·오디오편집·믹싱·
  마스터링, +기타는 전체 종류 그룹 드롭다운), 곡별 **진행 요약** 한 줄, 작업 행 = **헤더 접기 토글**(`<details>`, 우측 chevron·**상태 select는 그 앞 — 접힌 채로도 수정 가능**[헤더에 `<select form="task-form-<id>">`로 본문 폼에 연결, `[data-no-toggle]`+app.js click preventDefault로 펼침 방지]. **작업 상태=대기/완료 2단계**('진행중'[In_Progress] 폐기 2026-07-03 — `TASK_STATUSES`에서 제거, 레거시는 `task_status_drop_inprogress_v1` 마이그레이션+`normalizeTaskStatus`로 대기 승계); 펼치면 종류·담당·외주단가 편집 폼, **추가 시 자동 펼침** `?expand=`). **접힌 채 상태를 바꾸면(대기↔완료) 그 행이 자동으로 펼쳐진다**(2026-07-02) — 저장 버튼·'저장되지 않은 변경사항' 힌트가 본문에 있어 접힌 상태에선 안 보여 저장을 놓치던 문제 해결(app.js dirty 핸들러 `onEvt`: 변경 후 dirty면 `form.closest("details").open=true`, 사용자 피드백). 헤더의 종류 라벨·담당 엔지니어는 `data-row-type`/`data-row-engineer`로 **변경 즉시 갱신**(자동저장 응답의 typeLabel·engineerName). **후반작업은 전부 트랙/콘텐츠 고정·수량 1**(과금 유형·수량 선택 UI 폐기). **금액은 곡·콘텐츠 탭이 아니라 청구 탭에서 확정**(2026-07-01 — 세션 일정과 동일 철학: 작업 폼은 **종류·담당·상태만** 기록, 금액 칸 없음. 작업 생성·수정 시 종류 기본단가[`task_types.unit_price`]를 `total_price`에 **자동 적용**[`taskTypeUnitPrice`], 청구 생성 폼에서 작업별 금액 입력·조정해 최종 확정). 작업 행 헤더는 **청구된 작업의 확정액만** 표시(미청구는 숨김 — '기록만' 일관).
...
- 청구 탭 **청구 생성 폼**: **청구처 선택**(`clientCombo`, 기본값=프로젝트 자동파생값, 미선택 시 폴백 — 청구처 결정 지점) + **완료** 작업 + **청구 가능 녹음 세션**(녹음+단가+시간, 취소 제외)을 함께 체크박스로 노출 → 선택해 청구서로. **생성 즉시 발행**(폼에 안내); 발행/입금완료 인보이스의 **청구처 변경은 409 잠금**(미발행만 변경, `resolveInvoiceRefs`는 폼 선택 우선·미제공 시에만 프로젝트 폴백). **할인**(선택): 청구서 단위(정액 원·정률 %), **공급가에서 차감 후 VAT 재계산**(과세표준=공급가−할인, VAT=과세표준×10%, `invoiceAmountsFromSupply`); 공급가는 **체크한 항목 합으로 동적**(app.js `[data-discount-form]`·`data-line-amount`), 거래명세서는 소계→할인→과세표준→VAT→합계. `invoices.discount_amount`, from-tasks 주 경로(수동 인보이스는 표시용 저장). **공급가·VAT·총금액 카드를 발행일 위에 강조**(app.js 줄별 갱신). **부가세 포함 토글**(기본 체크, 해제 시 VAT 0=현금 거래, `vatIncluded`→`invoiceAmountsFromSupply`/`createInvoiceFromTasks`). **담당자(연락처)도 청구처 선택 가능**(`clientCombo`에 담당자 옵션·`payer_contact_id`→`ensureClientFromContact`로 **`source_contact_id` contact별 '기타' 클라이언트 매핑**[동명이인 병합 방지]·콤보 라벨에 소속·전화·#id 식별자). **④ 청구처 정합**(2026-07-02): 담당자로 선택한 연락처가 **개인 아티스트(source_contact_id로 연결된 '아티스트' 클라이언트)면 '기타' 셸 대신 그 아티스트 클라이언트를 청구처로 재사용** — 개인 아티스트 청구 시 아티스트+기타 이중 클라이언트가 생겨 청구·결제가 흩어지던 것 방지(`ensureClientFromContact` 선분기). **수동 인보이스도 VAT 포함 토글**(미체크 시 현금 VAT 0, 거래명세서 VAT 줄 생략)·**담당자 청구처 선택**(평면 select→콤보[클라이언트+담당자]·`payer_contact_id`→`resolveInvoiceRefs`에서 `ensureClientFromContact` 변환, from-tasks와 동일 UX).
  ...
  **녹음 세션은 곡·콘텐츠/버튼 없이 직접 청구**: **녹음 세션은 완료 안 됐어도(예정) 청구 후보에 노출**(2026-07-03 — 작업과 동일 규칙: 완료=기본 체크, 예정=흐리게·미체크·'예정' 태그, `listBillableSessionsForProject`/`computeInvoiceDraft` 조건 `status<>'취소'`). **예정 세션 체크 시 '완료로 바꿀까?' 확인**(data-confirm-pending) 후 **청구=완료 전환**(`createInvoiceFromTasks`가 `markSession`으로 status→'완료'), 선택 시 `invoice_items.session_id` 스냅샷으로 청구(곡·콘텐츠 안 거침). **세션 금액도 작업처럼 청구 폼에서 수정 가능**(`session_amount_<id>` 입력칸, 기본값=단가표 산정액 `s.billing.amount`; `data-line-input`로 공급가·VAT 실시간 갱신, 청구 시 입력값으로 `invoice_items` 스냅샷 확정[`createInvoiceFromTasks` `sessionAmounts`], **0원 가드 `TASK_AMOUNT_REQUIRED`**). 청구되면 세션 수정·삭제 잠금(`SESSION_INVOICED`), 인보이스 삭제 시 자동 미청구 복원. 관련: `listBillableSessionsForProject`(작성순 정렬 `created_at ASC`)·`unbilledInvoiceForm`·`createInvoiceFromTasks`(task+session 혼합)·`isSessionInvoiced`.
...
- **첨부 서류**(사업자등록증 — **통장사본 v1.0 폐기**, 스튜디오가 업체에 입금할 일 없음): 상세 폼에서 **드래그앤드롭/파일선택 업로드 + 열람**(`client_files`, 종류별 1개 교체식). 매직바이트 검증(PNG/JPEG/PDF, Content-Type 무시)·10MB·편집자 인증 다운로드(`inline`·`no-store`·**공개 링크 없음** — 민감 금융정보). `POST /clients/:id/files/:kind`·`GET .../raw`·`.../delete`, app.js `[data-dropzone]`. `bankbook` kind는 레거시(과거 업로드분 raw 열람만·업로드 UI 없음). **첨부는 업체(company)만 노출**(아티스트·그룹 제외, `clientFilesBlock`). **깨진 링크 감지**(2026-07-03): 상세 렌더 시 `storage.exists`(Drive=files.get·휴지통/404면 없음, 로컬=존재)로 실제 접근 가능 여부 확인 → 있을 때만 '보기', 없으면 '⚠️ 파일을 찾을 수 없습니다' + '기록 삭제'(dangling 기록 정리). 불확실(권한·네트워크)은 있음으로 간주(유효 파일 숨김 방지). 상세 라우트 async화.
- **분류 3분리 + 유형 먼저 선택 흐름**(2026-07-02): 클라이언트 목록 탭 = **업체 / 아티스트(개인 솔로) / 그룹(밴드·아이돌)** 3분류(상호배타, `isSoloArtist`=person·is_artist / 그룹=kind='group' / 업체=company). **'전체' 탭 폐기**(2026-07-03 — 기본 진입=업체). **'관계자' 탭 신설**(2026-07-03, 업체·아티스트 사이): 대표·A&R·담당자·디렉터·작가 등 **클라이언트 측 사람**(person·is_artist=0·비스태프·비외주 = `listAssociates`), 행=역할 배지(classifyParty)·현재 소속 회사·전화/이메일, **상세는 연락처**(`/contacts/:id`), 새 관계자=`/contacts/new`. 탭 순서=업체/관계자/아티스트/그룹. **'새 클라이언트'=드롭다운**(페이지 이동 없이 유형 선택, CSP-safe `<details>` 팝오버 — 업체/관계자/아티스트/그룹; app.js `[data-menu]` 바깥 클릭 닫기). `/clients/new` 폴백 페이지도 4카드. **목록 행 이름 뒤 메타**: 업체=대표, 아티스트·그룹=**소속사·소속 그룹**(text-xs muted, 업체 대표와 동일 톤; 배치 조회로 N+1 회피 — affiliations 소속사 + group_id 그룹명). '+ 새 클라이언트'는 **유형 3택 진입**(`GET /clients/new` → ?type 없으면 업체/아티스트/그룹 3카드, 있으면 유형별 폼) — 서로 섞이지 않는 3개념, **'기타' 폐기**. `clientForm(formType)`: **kind select 제거·유형 고정**(편집 시 유형 배지+'변경 불가', 유형은 party.kind라 불변). 업체=역할 체크·사업자·대표·주소·담당자 콤보·첨부, 아티스트=현금영수증·**소속 그룹 select**·**소속사 select**, 그룹=그룹명·**소속사 select**·**담당자 personCombo**(멤버는 상세; **현금영수증 없음** — 그룹엔 불필요, 2026-07-03. 편집 시 기존값 보존). **소속사 select**(아티스트·그룹, 업체 목록+'없음')는 `affiliations` 재사용(`setPartyAgency`: 회사 선택 시 이직 등록·'없음'이면 현재 소속 종료) → 소속사 상세 '소속 아티스트'에 자동 반영. **그룹↔멤버 소속사 연동**(2026-07-03): 멤버를 그룹에 추가하면 그룹 소속사를 **기본 상속**(`setPartyGroup` — 새로 소속될 때만, 같은 그룹 재저장 시 재상속 안 함), 그룹 소속사 변경 시 그 소속사를 **'따르던' 멤버(현재 소속사=그룹의 이전 소속사)에게만 전파**하고 **개별 지정(오버라이드)한 멤버는 유지**(`setPartyAgency` 그룹 분기·멤버 분기는 전파 없음). 폼 연동=소속 그룹 옵션 `data-agency`+app.js가 그룹 선택 시 소속사 select 자동 맞춤(이후 수동 변경 가능), `listGroupsForPicker`에 `agency_id` 포함. 회귀 테스트 party.test.js. `POST /`는 `type`(hidden)으로 `createCompany/createPerson[artist]/createGroup` 디스패치. **업체 역할 표기 '제작사'→'제작사/운영사'**(`companyRoleLabel` — 저장값은 `제작사` 유지, 프로젝트 파생·마이그레이션 호환; 폼 체크박스·목록/상세 배지 표시만 변경). app.js `[data-client-kind]` 토글 IIFE는 유형 고정으로 미사용(무해 no-op). `companyRolesFrom(b)` 단순화(체크된 roles CSV 또는 null).
...
- `track_tasks(track_id→project_tracks CASCADE, task_type, billing_type[Time_Charge|Fixed_Per_Track],
  ...
  worker_rate?, status[Pending|Completed],
