...
// ── 접속 IP(2026-07-20 사용자 요청 '아이피도 남기자') ──────────
test("clientIp: req.ip를 그대로, IPv4-mapped IPv6은 IPv4로 펴서", () => {
  const { clientIp } = require("../src/lib/audit");
  assert.equal(clientIp({ ip: "121.130.10.5" }), "121.130.10.5");
  // 같은 접속이 표기만 달라 다른 주소로 보이면 안 된다.
  assert.equal(clientIp({ ip: "::ffff:121.130.10.5" }), "121.130.10.5");
  assert.equal(clientIp({ ip: "2001:db8::1" }), "2001:db8::1", "진짜 IPv6은 그대로");
  assert.equal(clientIp({}), "", "IP를 못 얻어도 빈 문자열(예외 없음)");
  assert.equal(clientIp(null), "");
});

test("logAudit: ip는 target이 아니라 별도 컬럼에 들어간다", () => {
  clear();
  logAudit({ email: "who@gmail.com" }, "auth.deny", "미등록 계정 · 크롬/윈도우", "121.130.10.5");
  const r = rows("auth")[0];
  assert.equal(r.ip, "121.130.10.5");
  assert.ok(!/121\.130/.test(r.target), "target에는 안 섞인다(200자 절단·요약 성격 보존)");
});

test("logAudit: 파괴적·재무 액션은 ip 없이 기록된다(기존 호출부 무변경)", () => {
  clear();
  logAudit({ email: "chief@omgworks.kr" }, "invoice.delete", "OMG-202607-001");
  assert.equal(rows("work")[0].ip, null);
});

test("logAccessDaily: 접속 기록에도 ip가 남는다", () => {
  clear();
  logAccessDaily({ email: "ip@omgworks.kr", role: "chief" }, "크롬/맥", "10.0.0.9");
  assert.equal(rows("auth")[0].ip, "10.0.0.9");
});
